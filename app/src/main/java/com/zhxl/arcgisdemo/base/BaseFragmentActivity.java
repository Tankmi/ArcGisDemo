...
//		setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE); // 设置横屏加载
        ...
        if (isFullScreen) {    //全屏时添加一个占位的textview
...
//		mTitleLine = Mview.findViewById(R.id.iv_title_line);
        if (mBtnLeft != null) {
            mLayoutUtil.drawViewRBLayout(mBtnLeft, 88, 88, -1, 0, 0, 0);
    ...
    protected void reLoading() {
        IntentUtils.getInstance().reLogin(this);
    }
     ...
     * 设置状态栏颜色，默认透明
     *
     * @param isFullScreen: 是否是全屏显示	5.0以上的系统需要此属性判断状态栏的属性设置
     * @param isSetColor: 是否设置特殊颜色，否的话，第三个参数可以设置为0
     */
    public void setStatusBarColor(boolean isFullScreen, boolean isSetColor, int statusBarColor) {
        this.isFullScreen = isFullScreen;
        if (isSetColor) StatusBarCompat.compat(this, statusBarColor, isFullScreen);
        else
            StatusBarCompat.compat(this, mContext.getResources().getColor(R.color.status_bar_default_bg), isFullScreen);
    }
     ...
     *
     */
    public void setStatusBarColor(boolean isSetColor, int statusBarColor) {
        setStatusBarColor(false, isSetColor, statusBarColor);
    }

    /**
     *
     * @param statusBarColor
     * @param isDarkStyle   深色风格，默认是浅色风格
     */
    public void setStatusBarColor(int statusBarColor, boolean isFullScreen, boolean isDarkStyle) {
        StatusBarCompat.compat(this, statusBarColor, isFullScreen, isDarkStyle);
    }
